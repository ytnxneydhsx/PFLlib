...
class draw_slcdacp(drawbase):
    def __init__(self, log_filename=None):
        ...
        self.optimizer_str = None  # 初始化 optimizer_str
        ...
        if self.base_name and self.algorithm and self.model_str and self.dataset:
            ...
            optimizer_str = f"optimizer_{self.optimizer_str}" if self.optimizer_str is not None else "SGD" # 更新逻辑
    ...
    def plot_acc_img(self):
        ...
        print(f"准确率图表已保存: '{full_output_path}'")
        plt.close()
    ...
    def plot_loss_img(self):
        ...
        plt.grid(True); plt.tight_layout(); plt.savefig(full_output_path)
        print(f"损失图表已保存: '{full_output_path}'")
        plt.close()
    ...
    def plot_pruning_rate_img(self):
        ...
        print(f"剪枝率图表已保存: '{full_output_path}'")
        plt.close()
            ...
            if filename.endswith(".log") and filename.startswith("SLCDACP"):
                try:
                    ...
                    if not drawer.output_dir:
                        print(f"跳过文件 '{filename}'，无法确定输出目录。")
                        continue
                    drawer.plot_acc_img()
    ...
    def _load_parameters_from_logger(self):
        ...
        try:
            ...
            params = {
                ...
                'optimizer_str': r'optimizer_str\s*=\s*(\w+)'  # 添加 optimizer_str 的正则表达式
            }
            for key, pattern in params.items():
                match = re.search(pattern, content)
                if match:
                    val = match.group(1)
                    # 将 optimizer_str 作为字符串处理
                    if key in ['fixed_alpha', 'prune_tool', 'optimizer_str']: 
                        setattr(self, key, val)
                    elif key in ['alpha', 'local_learning_rate', 'purning_min', 'purning_base', 'purning_max', 'global_rounds']: 
                        setattr(self, key, float(val))
                    else: 
                        setattr(self, key, int(val))
        except Exception as e: print(f"解析日志文件时出错：{e}")

if __name__ == '__main__':
    processor = draw_slcdacp()
    processor.flatten_logs_and_cleanup_images() # 如果需要，可以取消这行注释来执行清理
